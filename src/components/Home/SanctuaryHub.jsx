/**
 * SanctuaryHub.jsx — v5.1
 *
 * Church command centre: service attendance + today's events + celebrations.
 * Brand: #0998d5 (blue) · #07c4b8 (teal) · #38bdf8 (sky)
 *
 * ─── v5.1 EVENT REFACTOR ────────────────────────────────────────────────
 *  • eventsData.data is null | single object — NOT an array.
 *  • filterTodayEvents() removed — no longer needed.
 *  • EventPanel now accepts `event` (object | null) instead of `events`.
 *  • EventCardInner enriched: streaming links (video/audio separately),
 *    registration link + deadline, start/end time range, end_date.
 *  • All other components, layout, and service logic untouched.
 * ────────────────────────────────────────────────────────────────────────
 */

import {
    memo, useCallback, useMemo, useRef, useState, useEffect,
} from 'react';
