...
// /** Translucent white glass for event card shell */
// const cardShell = () => ({
...
// /** Soft white depth for service rail inner elements */
...
// /** Format a 24-h time string "HH:MM:SS" → "10:30 AM" */
// const fmtTime24 = (t) => {
//     if (!t) return null;
...
// /** Rendered only inside ServiceRail header — single status source of truth */
...
// const BirthdayEntry = memo(({ person, index }) => {
...
//                 transition={{ delay: 0.035 * index, duration: 0.25 }}
//             >
...
// const AnniversaryEntry = memo(({ anniversary, person, index }) => {
//     const { isOpen, openModal, closeModal } = useModal();
//     return (
//         <>
//             <motion.div
...
//                     className="divide-y divide-gray-100 dark:divide-gray-700/60 max-h-[55vh] overflow-y-auto hub-scroll"
//                 >
...
//                         className="flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-bold text-white transition-all hover:opacity-90 active:scale-95"
...
//                             : <Cake size={13} style={{ color: '#93c5fd' }} />
//                         }
...
//                 {/* Streaming + Registration action buttons */}
...
// /**
//  * EventPanel — v5.1
//  * Accepts `event` (object | null) instead of `events` (array).
//  * isLive derived from event.status directly.
//  */
...
//             initial={{ opacity: 0, x: 14 }}
//             animate={{ opacity: 1, x: 0 }}
//             transition={{ duration: 0.48, ease: [0.22, 1, 0.36, 1], delay: 0.10 }}
...
//                     </div>
//                     <div>
//                         <h2 className="text-sm font-black text-white tracking-tight leading-tight">Today's Event</h2>
//                         {!isLoading && !isError && (
...
//             {/* Card */}
//             <div className="ev-card rounded-2xl overflow-hidden" style={cardShell()}>
//                 {isLoading ? (
...
//     /**
//      * eventsData.data is null | single event object — NOT an array.
//      * No filtering needed; pass directly to EventPanel.
//      */
...
//                     {/* EventPanel now receives single `event` object (or null) */}
//                     <EventPanel
//                         event={todayEvent}
//                         isLoading={eventsLoading}
//                         isError={eventsError}
...
import { memo, useCallback, useMemo, useRef, useState, useEffect } from 'react';
import { useSearchParams } from 'react-router-dom';
import { motion } from 'framer-motion';          // kept ONLY for HandIcon pulse button
import { useForm } from 'react-hook-form';
import Lottie from 'lottie-react';
import dayjs from 'dayjs';
import AOS from 'aos';
...
const HUB_CSS = `
  ...
  /* Scale-in spring — replaces framer-motion spring on checkmark circle */
  ...
  /* Celebrations button hover — replaces framer whileHover/whileTap */
  .hub-celeb-btn {
...
const BirthdayEntry = memo(({ person, index }) => {
    ...
    return (
        <>
            {/* AOS replaces motion.div fade-up with staggered delay */}
            <div
                data-aos="fade-up"
                data-aos-delay={Math.min(35 * index, 400)}
...
/* ─── Tab panel wrapper — CSS fade replaces AnimatePresence motion.div ─── */
const TabPanel = memo(({ tabKey, children }) => (
    // Re-keying via a wrapper with a unique key triggers remount → restarts animation
    <div
        key={tabKey}
...
const CelebrationsModal = memo(({
    ...
    return (
        ...
        >
            ...
            {/* CSS animation replaces AnimatePresence motion.div */}
            {tab === CELEBRATION_TABS.BIRTHDAYS ? (
...
const PageHeader = memo(({ user, isAuthenticated, totalCelebrations, isCoreLoading, onCelebrations }) => {
    ...
    return (
        /* AOS replaces motion.header fade-down */
        <header
            data-aos="fade-down"
        ...
        >
            ...
            <div className="flex items-center gap-2 sm:gap-3">
                {isAuthenticated && (
                    /* CSS hover/active replaces framer whileHover/whileTap */
                    <button
                        onClick={onCelebrations}
                        aria-label="View today's celebrations"
    ...
    <div className="hub-fade-in flex flex-col items-center gap-6">
        {!isPending ? (
            <div className="relative">
                ...
                {/* motion.button kept exclusively for HandIcon pulse animation */}
                <motion.button
                    onClick={onClockIn}
                    disabled={isPending}
                    className="relative rounded-full cursor-pointer disabled:opacity-50"
...
const AttendanceRecordedState = memo(({ attendance }) => (
    <div className="hub-fade-in flex flex-col items-center gap-5">
        {/* hub-scale-in CSS keyframe replaces framer-motion spring scale */}
...
const ServiceRail = memo(({
    ...
    const renderBody = () => {
        ...
        switch (status) {
            ...
            case SERVICE_STATUS.ONGOING:
                /* CSS fade replaces AnimatePresence; key swap forces remount → animation restart */
    ...
    return (
        /* AOS replaces motion.aside fade-right */
        <aside
            data-aos="fade-right"
...
const EventPanel = memo(({ event, isLoading, isError }) => {
    ...
    return (
        /* AOS replaces motion.section fade-left */
        <section
            data-aos="fade-left"
            data-aos-duration="480"
            data-aos-delay="100"
